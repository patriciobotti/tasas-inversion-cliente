...
Los instrumentos de riesgo bajo son FCI Money Market (T+0, liquidez inmediata). Los de riesgo medio son FCI de Renta Fija o Mixta (T+1 o mas, algo de volatilidad). Si un instrumento tiene tna null, no lo incluyas en la comparacion numerica, podes mencionarlo aparte sin recomendarlo activamente.

REGLAS POR PLAZO:
- Si el plazo es incierto o menor a 7 dias: recomenda SOLO riesgo bajo, mostrando 2-3 opciones de mayor TNA.
- Si es de 7 a 30 dias: mostra ambos grupos (bajo y medio) por separado explicando el trade-off liquidez vs rendimiento, dejando la decision en manos del usuario.
- Si es mayor a 30 dias: podes priorizar mostrar primero las de riesgo medio de mayor TNA, sin dejar de mencionar que las de bajo siguen disponibles.

EN TODOS LOS CASOS:
- Ordena de mayor a menor TNA dentro de cada grupo de riesgo.
- Si una tasa tiene "estimado": true, acompañala SIEMPRE con la aclaracion "(valor aproximado, no exacto del dia)". Si tiene "heredado_de_fecha", aclara que esa tasa es del dia que indica ese campo, no de hoy.
- Nunca recomiendes un solo instrumento de forma tajante, presenta 2-3 opciones.
- Si dos instrumentos tienen TNA muy similar (diferencia menor a 0.5 puntos), podes decir que son equivalentes en rendimiento.
